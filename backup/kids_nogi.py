...
# with open("source/chebyrashka.html", "w", encoding="UTF-8") as file:
#     file.write(src)
...
try:
    ...
    for name in nazvaniya:
        with open(f"{folder_name}/{name}", "r", encoding="UTF-8") as file:
            ...
            discr = soup.find("div", id="tab-description")
            try:
                razm = discr.find("p").text
                # razm = razm.replace("<p>","")
                razm = razm.strip()
            except:
                razm = "-"
            ...
            try:
                opis = discr.find("p").findNext("p").text
                opis = opis.strip()
            except:
                opis = "-"
