...
domen = "https://chebyrashka.com.ua/ru/detskaya-odezhda-dlya-malchikov?limit=300"
...
with open("data/boys.html", "w", encoding="UTF-8") as file:
    file.write(src)
...
try:
    ...
    with open("data/boys.html", "r", encoding="UTF-8") as file:
        src = file.read()
    ...
    for i in products_urls:
        ...
        b = i.split("/")
        with open(f"data/boys/{n}_{b[4]}.html", "w", encoding="UTF-8") as file:
            file.write(src)
...
try:
    ...
    for name in nazvaniya:
        with open(f"{folder_name}/{name}", "r", encoding="UTF-8") as file:
            ...
            with open("data/boys.csv", "a", newline='', encoding="UTF-8") as file:
